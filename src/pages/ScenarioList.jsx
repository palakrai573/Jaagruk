...
export default function ScenarioList() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 py-10">
      <p className="font-mono text-brand-text text-xs tracking-[0.2em] uppercase mb-3">{t('list_eyebrow')}</p>
      <h1 className="font-display font-bold text-4xl md:text-5xl uppercase mb-2">{t('list_title')}</h1>
      <p className="text-ink-tertiary mb-8 max-w-xl leading-relaxed">{t('list_desc')}</p>

      {/* Progress toward certification */}
      {!loading && (
        <div className="bg-surface-1 border border-line-subtle rounded-lg p-5 mb-8 flex items-center justify-between gap-4 flex-wrap">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 flex-wrap font-mono text-[10px] uppercase tracking-widest">
                ...
                {!counts && <span className="text-ink-tertiary">· {t('as_grade_unknown')}</span>}
                {meta.smoke > 0 && (
      ...
      <Link
        to="/buddy"
        className="mt-6 block bg-surface-1 border border-line-subtle rounded-lg p-6 hover:border-brand transition-colors"
      >
